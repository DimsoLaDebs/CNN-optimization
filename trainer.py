...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            average_loss += loss.item() * X_batch.size(0)  # Multiply by batch size

            # Compute Accuracy
            _, predicted_labels = torch.max(output_probs, 1)
            accuracy += (predicted_labels == Y_batch).sum().item()
...
class Trainer:
    ...
    def __init__(self,
        ...
        #self.optimizer = torch.optim.RMSprop(self.model.parameters(), self.learning_rate, weight_decay=1e-6)  
        self.optimizer = torch.optim.AdamW(self.model.parameters(), self.learning_rate, weight_decay=1e-6) # For Resnet
